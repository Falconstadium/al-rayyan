export const lemonHoneyDetails = {
  en: {
    name: "lemon honey",
    slug: "lemon-honey",

    text: "It is the honey produced by bees from the nectar of lemon and orange blossoms, it is usually light-dense, light yellow color tilted to golden, it tastes great light, it has a smell similar to citrus fruits. The nutritional value of lemon honey is rich in flying oils, with many benefits for human health.",

    benefit: {
      title: "Benefits of Lemon Honey:",
      detail1:
        "Improves digestive performance, helps to activate digestion in the gut, prevents constipation and treats diarrhoea, in addition to its role in expelling gases, treating acidity, and preventing the occurrence of intestinal colic in children.",
      detail2:
        "Enhances the immune system's work and increases its ability to resist various diseases, especially colds and flu.",
      detail3:
        "It activates brain action, improves mental abilities, strengthens memory, improves assimilation and prevents Alzheimer's infection in the elderly, contributes to strengthening nerves and muscles, is effective in calming nerves, and gives the body a sense of relaxation.",
    },
  },

  fr: {
    name: "miel de citron",
    slug: "miel-de-citron",

    text: "C’est le miel produit par les abeilles à partir du nectar des fleurs de citron et d’orange, il est généralement léger-dense, jaune clair de couleur inclinée à l’or, il a un goût de grande lumière, il a une odeur semblable aux agrumes. La valeur nutritive du miel de citron est riche en huiles volantes, avec de nombreux avantages pour la santé humaine.",

    benefit: {
      title: "Avantages du miel de citron :",
      detail1:
        "Améliore les performances digestives, aide à activer la digestion dans l’intestin, prévient la constipation et traite la diarrhée, en plus de son rôle dans l’expulsion des gaz, le traitement de l’acidité et la prévention de la colique intestinale chez les enfants.",
      detail2:
        "Améliore le travail du système immunitaire et augmente sa capacité à résister à diverses maladies, en particulier les rhumes et la grippe.",
      detail3:
        "Il active l’action du cerveau, améliore les capacités mentales, renforce la mémoire, améliore l’assimilation et prévient l’infection d’Alzheimer chez les personnes âgées, contribue à renforcer les nerfs et les muscles, est efficace pour calmer les nerfs et donne au corps un sentiment de relaxation.",
    },
  },

  ar: {
    name: "عسل الليمون",
    slug: "عسل-الليمون",

    text: "إنه العسل الذي ينتجه النحل من رحيق أزهار الليمون والبرتقال، وعادة ما يكون كثيفًا باللون الأصفر الفاتح مائلًا إلى الذهبي، ومذاقه خفيف للغاية، ورائحته تشبه رائحة الحمضيات. القيمة الغذائية لعسل الليمون غنية بالزيوت الطائرة، مع العديد من الفوائد لصحة الإنسان.",

    benefit: {
      title: "فوائد عسل الليمون:",
      detail1:
        "يحسن أداء الجهاز الهضمي، ويساعد على تنشيط الهضم في الأمعاء، ويمنع الإمساك ويعالج الإسهال، بالإضافة إلى دوره في طرد الغازات، وعلاج الحموضة، ومنع حدوث المغص المعوي لدى الأطفال.",
      detail2:
        "يعزز عمل الجهاز المناعي ويزيد من قدرته على مقاومة الأمراض المختلفة، وخاصة نزلات البرد والإنفلونزا.",
      detail3:
        "إنه ينشط حركة الدماغ، ويحسن القدرات العقلية، ويقوي الذاكرة، ويحسن الاستيعاب ويمنع عدوى الزهايمر لدى كبار السن، ويساهم في تقوية الأعصاب والعضلات، ويكون فعالًا في تهدئة الأعصاب، ويمنح الجسم إحساسًا بالاسترخاء.",
    },
  },
};
